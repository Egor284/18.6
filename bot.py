...
def convert(message: telebot.types.Message):
    try:
        ...
        if len(values) != 3:
            raise ConvertionException(f'Введите по шаблону,слишком много параметров.')
        ...
        bot.reply_to(message, f'Не удалось обработатб команду\n{e}')
    else:

        text = f'За {amount} {quote} дают {total_base} {base}'
